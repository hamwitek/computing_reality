...
def generate_3dmfile(png_image_path, tiff_image_path, json_file, output_dir=None):
    """
    ...
    try:
        ...
        if output_dir is None:
            output_dir = os.path.dirname(tiff_image_path)

        # Generate unique filenames
        unique_id = uuid.uuid4().hex[:8]
        output_image_path = os.path.join(
            output_dir, f"tree_detection_results_{unique_id}.png")
        output_rhino_file = os.path.join(
            output_dir, f"tree_detection_{unique_id}.3dm")
        ...
        boxes.root_dir = os.path.dirname(tiff_image_path)
        ...
        if boxes.empty:
            print(
                "⚠️ No trees detected! Try lowering the threshold or using a different image.")
        else:
            ...
            output_image_path = os.path.join(os.path.dirname(
                tiff_image_path), "tree_detection_results.png")
        ...
        curve = rhino3dm.PolylineCurve(
            [corner1, corner2, corner3, corner4, corner1])
        ...
        material.SetBitmapTexture(png_image_path)  # Attach image
        ...
        for _, row in boxes.iterrows():
            ...
            norm_x = (row['center_x'] / image.shape[1]) * \
        ...
        # Save Rhino file - moved outside the loop
        model.Write(output_rhino_file, 6)

        # Return results
        # return {
        #     "status": "success",
        #     "message": f"Successfully detected {len(boxes)} trees and created 3DM file",
        #     "rhino_file": output_rhino_file,
        #     "tree_count": len(boxes),
        #     "detection_image": output_image_path
        # }
